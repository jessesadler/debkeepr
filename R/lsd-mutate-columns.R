...
## Librae ##
deb_librae <- function(l, s, d, bases = c(20, 12), round = 5) {
  if (length(l) > 1) {
    return(purrr::pmap_dbl(list(l, s, d), ~ deb_librae(..1, ..2, ..3, bases, round)))
  }
  ...
  if (!is.na(librae) &&
      dplyr::near(round(lsd[3] %% bases[2], round), bases[2]) &&
...
## Solidi ##
deb_solidi <- function(l, s, d, bases = c(20, 12), round = 5) {
  if (length(l) > 1) {
    return(purrr::pmap_dbl(list(l, s, d), ~ deb_solidi(..1, ..2, ..3, bases, round)))
  }
  ...
  if (!is.na(solidi) && dplyr::near(round(lsd[3] %% bases[2], round), bases[2])) {
    solidi <- solidi + 1
...
## Denarii ##
deb_denarii <- function(l, s, d, bases = c(20, 12), round = 5) {
  if (length(l) > 1) {
    return(purrr::pmap_dbl(list(l, s, d), ~ deb_denarii(..1, ..2, ..3, bases, round)))
  }

  lsd <- lsd_decimal_check(c(l, s, d), bases)

  denarii <- round(lsd[3] %% bases[2], round)
...
lsd_mutate_columns <- function(df,
  ...
  if (replace == FALSE) {
    dplyr::mutate(df,
                  !! lsd_names[1] := deb_librae(!! l, !! s, !! d, bases, round),
                  !! lsd_names[2] := deb_solidi(!! l, !! s, !! d, bases, round),
                  !! lsd_names[3] := deb_denarii(!! l, !! s, !! d, bases, round))
  } else {
    ret <- dplyr::mutate(df,
                         temp_librae_col = deb_librae(!! l, !! s, !! d, bases, round),
                         temp_solidi_col = deb_solidi(!! l, !! s, !! d, bases, round),
                         temp_denarii_col = deb_denarii(!! l, !! s, !! d, bases, round)) %>%
