...
#'
#' `deb_account2()` is similar to [deb_account_summary2()], but it only returns
#' the information for one account instead of all accounts in the credit and/or
#' debit variables of a transactions data frame.
#'
#' `deb_account2()` is part of a family of functions meant to be used on
...
deb_account2 <- function(df, account_id,
...
#'
#' `deb_account_summary2()` is similar to [deb_account2()], but it returns the
...
#' [deb_current2()].
#'
#' `deb_account_summary2()` is part of a family of functions meant to be used on
#' data frames that contain transactions between accounts in an account
#' book. The data frame should possess a similar structure to a
#' [directed edge list](https://www.jessesadler.com/post/network-analysis-with-r/#nodes-edges).
#' In this context, credit and debit variables contain the ids of the accounts
...
deb_account_summary2 <- function(df,
                                 credit = credit, debit = debit,
                                 lsd = lsd,
                                 round = 5,
                                 na.rm = FALSE) {
...
#' lsd list column.
#'
#' `deb_credit2()` is similar to [deb_account_summary2()], but it only returns
#' the credit values for the accounts in `df`. See [deb_debit2()] to return
#' the debit totals for the accounts in `df`.
#'
#' `deb_credit2()` is part of a family of functions meant to be used on
#' data frames that contain transactions between accounts in an account
#' book. The data frame should possess a similar structure to a
#' [directed edge list](https://www.jessesadler.com/post/network-analysis-with-r/#nodes-edges).
#' In this context, credit and debit variables contain the ids of the accounts
...
#' deb_credit2(df = trans,
#'             credit = credit,
#'             lsd = lsd)
#'
#' @export

deb_credit2 <- function(df,
                        credit = credit,
                        lsd = lsd,
                        round = 5,
                        na.rm = FALSE) {
...
#' lsd list column.
#'
#' `deb_debit()2` is similar to [deb_account_summary2()], but it only returns
#' the debit values for the accounts in `df`. See [deb_credit2()] to return
#' the credit totals for the accounts in `df`.
#'
#' `deb_debit()2` is part of a family of functions meant to be used on
#' data frames that contain transactions between accounts in an account
#' book. The data frame should possess a similar structure to a
#' [directed edge list](https://www.jessesadler.com/post/network-analysis-with-r/#nodes-edges).
#' In this context, credit and debit variables contain the ids of the accounts
...
#'
#' `deb_current2()` is similar to [deb_account_summary2()], but it only returns
...
#'  zero — see [deb_open2()].
#'
#' `deb_current2()` is part of a family of functions meant to be used on
#' data frames that contain transactions between accounts in an account
#' book. The data frame should possess a similar structure to a
#' [directed edge list](https://www.jessesadler.com/post/network-analysis-with-r/#nodes-edges).
#' In this context, credit and debit variables contain the ids of the accounts
...
#' @family lsd account functions
#'
#' @inheritParams deb_account_summary2
#'
#' @return Returns a tibble with one row for each account in `df` with the
...
#' deb_current2(df = trans,
#'              credit = credit,
#'              debit = debit,
#'              lsd = lsd)
#' @export

deb_current2 <- function(df,
                         credit = credit, debit = debit,
                         lsd = lsd,
                         round = 5,
                         na.rm = FALSE) {
...
#' as negative values.
#'
#' `deb_open()2` is similar to [deb_current2()], but it only returns current
#' values for accounts that have a value that is not zero.
#'
#' `deb_open2()` is part of a family of functions meant to be used on
#' data frames that contain transactions between accounts in an account
#' book. The data frame should possess a similar structure to a
#' [directed edge list](https://www.jessesadler.com/post/network-analysis-with-r/#nodes-edges).
#' In this context, credit and debit variables contain the ids of the accounts
...
#'
#' `deb_balance2()` is based on [deb_open2()]. The function sums the credits
...
#'
#' `deb_balance2()` is part of a family of functions meant to be used on
#' data frames that contain transactions between accounts in an account
#' book. The data frame should possess a similar structure to a
#' [directed edge list](https://www.jessesadler.com/post/network-analysis-with-r/#nodes-edges).
#' In this context, credit and debit variables contain the ids of the accounts
