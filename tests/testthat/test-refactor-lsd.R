...
test_that("l, s, and d must be the same length", {
  expect_error(deb_refactor(l = c(9, 0), s = 8, d = 2),
...
answer_tbl <- tibble::tibble(l = c(9, 10, -10, 11),
                             s = c(18, 5, -5, 17),
                             d = c(1, 5.2, -5.2, 9.9))
answer_list <- list(deb_refactor(8, 36, 25, vector = TRUE),
                    deb_refactor(8.325, 36.85, 25, vector = TRUE),
                    deb_refactor(-8.325, -36.85, -25, vector = TRUE),
                    deb_refactor(10, 36.325, 18, vector = TRUE))
...
test_that("vectorization of deb_refactor works", {
  # tibble output
  expect_equal(nrow(deb_refactor(l_vector, s_vector, d_vector)), 4)
  expect_equal(deb_refactor(l_vector, s_vector, d_vector), answer_tbl)
  # vector or list output
  expect_equal(length(deb_refactor(l_vector, s_vector, d_vector, vector = TRUE)), 4)
  expect_equal(is.list(deb_refactor(8, 35, 25, vector = TRUE)), FALSE)
  expect_equal(is.list(deb_refactor(l_vector, s_vector, d_vector, vector = TRUE)), TRUE)
  expect_equal(deb_refactor(l_vector, s_vector, d_vector, vector = TRUE), answer_list)
})
