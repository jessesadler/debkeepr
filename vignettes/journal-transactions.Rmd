---
title: "2. Transactions in Richard Dafforne's Journal"
author: "Jesse Sadler"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{2. Transactions in Richard Dafforne's Journal}
...
The `debkeepr` package provides an interface for analyzing non-decimal currencies that use the tripartite system of pounds, shillings, and pence. The use of non-decimal currencies in Europe in the medieval and early modern period complicates the calculation and analysis of monetary transactions in the past. This vignette uses examples taken from the practice journal in the third edition of Richard Dafforne’s *The Merchant's Mirrour, Or Directions for the Perfect Ordering and Keeping of His Accounts* (London, 1660) to show how `debkeepr` can help to explicate and simplify monetary transactions that involve a variety of non-decimal currencies.
...
To find the total net weight of the kettles we can add the gross weights and tare weights and then subtract the two values. This can be done by transforming the gross and tare values to `lsd` objects with bases of 4 and 28. A shortcut for getting the sum of the tare values is to add them together within `deb_d_lsd()`, because the pounds unit corresponds to the pence unit. The various values are saved as objects and printed to the console to demonstrate their values.

```{r 3. kettles weight}
# 3. Gross weight
kettles_gross <- deb_as_lsd(lsd = list(c(2, 3, 26),
                                       c(2, 2, 18),
                                       c(2, 1, 21),
                                       c(2, 0, 17),
                                       c(1, 3, 5)),
...
A merchant involved in long-distance trade often had to record the value of transactions that occurred two currencies that differed from the money of account used in the account book. Such a situation appeared in transaction 58 in which Jacob Symonson in Amsterdam received a bill of exchange from Jean du Boys in Rouen that consisted of capital for a company involving the bookkeeper and Randoll Rice. Jean du Boys sent Symonson £1,140 17s. 8d. French crowns at the rate of 123d. Flemish, and this transaction was calculated at the rate of 72d. sterling for the purposes of accounting in terms of pounds sterling. In this case, a bookkeeper needed to know the amount of guilders received by Symonson, which was calculated through pounds Flemish, as well as the value in pounds sterling.
...
[^2]:	England used the Julian calendar until 1752, making it ten days behind the Gregorian calendar throughout the 17th century. In addition, the English calendar used 25 March as the start of the new year. Thus, 1 January 1633 was actually 11 January 1634 by the Gregorian calendar.
