...
Note that `deb_lsd` vectors are not currently fully integrated into `dplyr`. `dplyr` functions such as `summarise()`, `mutate()`, and `arrange()` do not work with `deb_lsd` columns. The same is true for plotting with `ggplot2`. However, these limitations can be overcome by converting `deb_lsd` values to `deb_decimal`. The following code simplifies the `dafforne_transactions` data frame and adds a `deb_decimal` column.
...
The account books only showed three small losses or debits during the course of the books, so it is more interesting to concentrate on the much more substantial profits. A useful measurement is to see not only the raw value that each account created in profits but also the relation of that value to the total profits, shown here as a percentage. `dplyr`'s `summarise()` does not work with `deb_lsd` columns, and so the `deb_decimal` column is used instead. However, an equivalent `deb_lsd` column is recreated with base R methods, so that the profit values appear as they do in the book.
...
```{r growth percentage}
# Create an lsd vector for easier calculation
