...
Note that `deb_lsd` objects are not currently fully integrated into `dplyr`. This means that `dplyr` functions such as `summarise()`, `mutate()`, and `arrange()` do not work with `deb_lsd` columns. The same is true for plotting with `ggplot2`. However, you can get around these limitations by converting `deb_lsd` values to `deb_decimal`. The following code simplifies the `dafforne_transactions` data frame and adds a `deb_decimal` column.
